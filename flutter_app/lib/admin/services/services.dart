...
class AdminDashboardService {
  ...
  Future<FullDriver> fetchEachDriver(String driverID) async {
    final response = await http
        .get(Uri.parse('http://10.0.2.2:5000/admin/drivers/$driverID'));
    if (response.statusCode == 200) {
      Map<String, dynamic> driverDataFull = json.decode(response.body);
      if (driverDataFull['SDT'] != null) {
        String driverID = driverDataFull['Driver_ID'].toString();
        String driverWallet = driverDataFull['Wallet'].toString();
        String driverExperiment =
            driverDataFull['Working_experiment'].toString();
        return FullDriver(
            driverID,
            driverDataFull['SDT'],
            driverDataFull['Firstname'],
            driverDataFull['Lastname'],
            driverWallet,
            driverDataFull['DOB'],
            driverDataFull['Gender'],
            driverDataFull['Address'],
            driverDataFull['CCCD'],
            driverDataFull['Driving_licence_number'],
            driverExperiment);
      } else {
        print('Null data received for the driver with ID: $driverID');
        throw Exception('Null data received for the driver');
...
/*
  Future<List<FullDriver>> fetchEachDriver() async {
    final response =
        await http.get(Uri.parse('http://10.0.2.2:5000/admin/drivers'));
    if (response.statusCode == 200) {
      List<dynamic> driversEachData = json.decode(response.body);
      List<FullDriver> driverInfomation = [];
      for (var driverEachData in driversEachData) {
        if (driverEachData['SDT'] != null) {
          String driverID = driverEachData['Driver_ID'].toString();
          String driverWallet = driverEachData['Wallet'].toString();
          String driverExperiment =
              driverEachData['Working_experiment'].toString();
          driverInfomation.add(FullDriver(
              driverID,
              driverEachData['SDT'],
              driverEachData['Firstname'],
              driverEachData['Lastname'],
              driverWallet,
              driverEachData['DOB'],
              driverEachData['Gender'],
              driverEachData['Address'],
              driverEachData['CCCD'],
              driverEachData['Driving_licence_number'],
              driverExperiment));
        } else {
          print('Null data received for a driver.');
        }
...
class AdminDashboardService {
  Future<List<Driver>> fetchDrivers() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Driver> drivers = [];
      for (var driverData in driversData) {
        drivers.add(Driver(driverData['Họ tên'], driverData['CCCD']));
      }
      return drivers;
    } else {
      throw Exception('Failed to load drivers');
